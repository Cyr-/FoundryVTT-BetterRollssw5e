...
export async function migrate() {
	const lastVersion = game.settings.get("betterrollssw5e", "migration")?.version;
	...
	try {
		...
		if (isNewerVersion("1.6.12", lastVersion)) {
			numItemsUpdated += await iterAndUpdateItems((item) => {
				const updates = {};
				const brFlags = item.data.flags.betterRollssw5e;
				if (!brFlags) return;
				...
				if ("critRange" in brFlags) {
					updates["flags.betterRollssw5e.-=critRange"] = null;
				}

				if ("critDamage" in brFlags) {
					updates["flags.betterRollssw5e.-=critDamage"] = null;
				}
		...
		game.settings.set("betterrollssw5e", "migration", {
			status: true,
...
export async function iterAndUpdateItems(callback) {
	...
	for (const item of game.items) {
		...
		if (updates) {
			console.log(`BetterRollssw5e | Migrating world ${item.name}`);
	...
	for (const actor of game.actors) {
		...
		for (const item of actor.items) {
			...
			if (update) {
				console.log(`BetterRollssw5e | Migrating ${item.name} on actor ${actor?.name}`);
